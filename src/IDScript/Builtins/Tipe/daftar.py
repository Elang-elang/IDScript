...
def masukkan(ini: Daftar, value: Any, index: int = -1) -> None:
    ...
    if bool(ini.const_length) is True:
        if index > ini.const_length:
            raise IndexError(f'Panjang telah ditetapkan {index} melebihi {ini.const_length}')
...
def cek_jangkauan(ini: Daftar, jangkauan: Any) -> int:
    res = [isinstance(Daftar, jangkauan)]
    if not all(res): return 0

    res.append(ini.frozen       is jangkauan.frozen)
    res.append(ini.const_length is jangkauan.const_length)
    res.append(ini.values       is jangkauan.values)
    return int(all(res))
...
def tulisan(ini: Daftar) -> str:
    writer = ini.ke_teks()
    return f"Daftar({writer})" if not ini.frozen else f'Daftar<Tetap>({writer})'

@Daftar.Method(public=True)
def ke_teks(ini: Daftar) -> str:
    new_v = []
    for v in ini.values:
        if v is ini.values:
            new_v.append('[ ... ]')
