...
class Daftar(Iterable):
    ...
    def _sort_key(value, self_sentinel=_EMPTY):
        ...
        from IDScript.builtins.Kamus import Kamus
    ...
    @staticmethod
    def adalah_daftar(instance: Any):
        return isinstance(instance, (Daftar, list))
...
@IDSFunction(name="Daftar", arguments={"nilai": Any}, annotation=Any)
def _daftar(nilai: Any = _EMPTY) -> Daftar:
    if nilai is _EMPTY:
        return Daftar()
    return Daftar(nilai)


@IDSFunction(name="adalah_daftar", arguments={"nilai": Any}, annotation=bool)
def _adalah_daftar(nilai: Any) -> bool:
    return isinstance(nilai, (list, Daftar))


@IDSModule(name="Daftar", path=Path(__file__).with_suffix(".idsm"))
def module(cls):
    cls.add(_daftar, _adalah_daftar)
    cls.typedef('TDaftar', Daftar, params=['T'])
