...
@IDSFunction(name="buat_server", arguments={"host": str, "port": int, "routes": dict}, annotation=Any)
...
@IDSFunction(name="jalankan", arguments={"server": Any}, annotation=None)
def _jalankan(server: Any) -> None:
    unwrap_py_value(server).serve_forever()


@IDSFunction(name="jalankan_thread", arguments={"server": Any}, annotation=Any)
...
@IDSFunction(name="hentikan", arguments={"server": Any}, annotation=None)
...
@IDSFunction(name="respons", arguments={"teks": str, "status": int, "headers": dict}, annotation=dict)
def _respons(teks: str, status: int, headers: dict) -> dict:
    return {"body": teks, "status": int(status), "headers": dict(headers)}


@IDSModule(name="HTTP", path=Path(__file__).with_suffix(".idsm"))
def module(cls):
