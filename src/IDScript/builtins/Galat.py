...
@IDSFunction(name="Galat", arguments={"message": Any}, annotation=Any)
def _galat(message) -> IDSError:
    return IDSError(message)


@IDSFunction(name="GalatTipe", arguments={"message": Any}, annotation=Any)
def _galat_tipe(message) -> IDSTypeError:
    return IDSTypeError(message)


@IDSFunction(name="GalatNilai", arguments={"message": Any}, annotation=Any)
def _galat_nilai(message) -> IDSValueError:
    return IDSValueError(message)


@IDSFunction(name="GalatAtribut", arguments={"message": Any}, annotation=Any)
def _galat_atribut(message) -> IDSAttributeError:
    return IDSAttributeError(message)


@IDSFunction(name="GalatIndeks", arguments={"message": Any}, annotation=Any)
def _galat_indeks(message) -> IDSIndexError:
    return IDSIndexError(message)


@IDSFunction(name="GalatKunci", arguments={"message": Any}, annotation=Any)
def _galat_kunci(message) -> IDSKeyError:
    return IDSKeyError(message)


@IDSFunction(name="GalatModul", arguments={"message": Any}, annotation=Any)
def _galat_modul(message) -> IDSModuleError:
    return IDSModuleError(message)


@IDSFunction(name="GalatLoop", arguments={"message": Any}, annotation=Any)
def _galat_loop(message) -> IDSLoopError:
    return IDSLoopError(message)


@IDSFunction(name="GalatMemori", arguments={"message": Any}, annotation=Any)
def _galat_memori(message) -> IDSMemoryError:
    return IDSMemoryError(message)


@IDSModule(name="Galat", path=Path(__file__).with_suffix(".idsm"))
def module(cls):
    cls.add(
        _galat,
        _galat_tipe,
