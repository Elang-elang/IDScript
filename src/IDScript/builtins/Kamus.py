...
from IDScript.builtins.Daftar import Daftar
...
class Kamus(MutableMapping):
    ...
    @staticmethod
    def adalah_kamus(instance: Any):
        return isinstance(instance, (Kamus, dict, MutableMapping, Mapping))
...
@IDSFunction(name="Kamus", arguments={"nilai": Any}, annotation=Any)
def _kamus(nilai: Any) -> Kamus:
    return Kamus(nilai)


@IDSFunction(name="adalah_kamus", arguments={"nilai": Any}, annotation=bool)
def _adalah_kamus(nilai: Any) -> bool:
    return isinstance(nilai, (dict, Kamus))


@IDSModule(name="Kamus", path=Path(__file__).with_suffix(".idsm"))
def module(cls):
    cls.add(_kamus, _adalah_kamus)
    cls.typedef('TKamus', Kamus, params=['K', 'V'])
